...
# Set up paths/urls
define('Web2VPN_ROOT', str_replace('\\', '/', dirname(dirname(__FILE__))));
define('Web2VPN_URL',
	'http'
	. ( HTTPS ? 's' : '' )
	. '://'
...
define('Web2VPN_BROWSE', Web2VPN_URL . '/' . SCRIPT_NAME);
...
# Load settings
require Web2VPN_ROOT . '/includes/settings.php';
...
# set the MULTIWeb2VPN constant to stop the script automatically loading theme config files.
if ( ! defined('MULTIWeb2VPN') && file_exists($tmp = Web2VPN_ROOT . '/themes/' . $CONFIG['theme'] . '/config.php') ) {
...
function proxyURL($url, $givenFlag = false) {
	...
	# Validate the input
	if ( empty($url) || $url[0]=='#' || $url=='about:' || stripos($url,'data:')===0 || stripos($url,'file:')===0 || stripos($url,'res:')===0 || stripos($url,'C:')===0 || strpos($url, Web2VPN_BROWSE)===0 ) {
		return '';
	}
	...
	if ( $CONFIG['path_info_urls'] && $options['encodeURL'] ) {
		return Web2VPN_BROWSE . '/' . str_replace('%', '_', chunk_split($url, 8, '/')) . 'b' . $bitfield . '/' . ( $addFlag ? 'f' . $addFlag : '') . $anchor;
	}

	# Otherwise, return in 'normal' (query string) format
	return Web2VPN_BROWSE . '?u=' . $url . '&b=' . $bitfield . ( $addFlag ? '&f=' . $addFlag : '' ) . $anchor;
}
...
function deproxyURL($url, $verifyUnique=false) {
	...
	# Remove our prefix
	$url = str_replace(Web2VPN_BROWSE, '', $url);
...
function getTemplatePath($file) {
	...
	if ( ! file_exists($return = Web2VPN_ROOT . '/themes/' . $CONFIG['theme'] . '/' . $file . '.php') ) {

		# Then look in default folder (if different)
		if ( $CONFIG['theme'] == 'default' || ! file_exists($return = Web2VPN_ROOT . '/themes/default/' . $file . '.php') ) {
	...
	if ( ! empty($themeReplace) ) {
		...
		foreach ( $themeReplace as $tag => $value ) {
			...
			if ( COMPATABILITY_MODE ) {
				$template = str_replace('<!--[Web2VPN:' . $tag . ']-->', $value, $template);
			}
...
function injectionJS() {
	...
	$jsFile = Web2VPN_URL . '/includes/main.js?'.$CONFIG['version'];
...
function currentURL() {
	...
	# Return full URL
	return Web2VPN_BROWSE . $separator . ( isset($_SERVER[$method]) ? $_SERVER[$method] : '');
}
