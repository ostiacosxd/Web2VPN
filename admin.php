...
*******************************************************************
* This is a stand-alone admin control panel for the Web2VPN software.
******************************************************************/
...
# this admin script out of the Web2VPN directory. You can use a relative
# or absolute path to the file.
define('ADMIN_Web2VPN_SETTINGS', 'includes/settings.php');
...
# Set up equivalents to Web2VPN's /includes/init.php constants
# that might be available in the settings file
define('Web2VPN_URL',	pathToURL(dirname(ADMIN_Web2VPN_SETTINGS) . '/..'));
define('Web2VPN_ROOT', str_replace('\\', '/', dirname(dirname(realpath(ADMIN_Web2VPN_SETTINGS)))));

# And backwards compatibility (will be removed at some point)
function findURL() { return Web2VPN_URL; }
define('LCNSE_KEY', '');
define('proxyPATH', Web2VPN_ROOT . '/');

# Load current settings
$settingsLoaded = file_exists(ADMIN_Web2VPN_SETTINGS) && (@include ADMIN_Web2VPN_SETTINGS);

# Extract the "action" from the query string
$action = isset($_SERVER['QUERY_STRING']) && preg_match('#^([a-z-]+)#', $_SERVER['QUERY_STRING'], $tmp) ? $tmp[1] : '';

$cache_bust=filemtime(__FILE__)+filemtime(ADMIN_Web2VPN_SETTINGS);
...
class SkinOutput extends Output {
	...
	protected function wrap() {
		...
		# Append "Web2VPN control panel" to title
		$title = $this->title . ( $this->title ? ' : ' : '' ) . 'Web2VPN control panel';
...
OUT;
		...
		echo <<<OUT
				...
				<div id="topheader">
					<h1 id="title">
						<a href="{$self}">Web2VPN Admin Control Panel</a><br>
...
OUT;
		...
		<div id="footer">
			...
			<div id="footer_bg">
				<p><a href="http://www.ihoster.us/">Web2VPN</a>&reg; &copy; 2007-2015 Web2VPN. All rights reserved.</p>
			</div>
...
if ( $input->gFetch && $user->isAdmin() ) {
	...
	switch ( $input->gFetch ) {
		...
		case 'news':
			...
			# Connect to Web2VPN
			if ($ch=curl_init('http://www.ihoster.us/feeds/news.php?vn='.urlencode($CONFIG['version']).'&lk='.urlencode($CONFIG['license_key']).'&cb='.$cache_bust)) {
...
if ( ! $settingsLoaded ) {
	...
	# Show error and exit
	$error->add('The settings file for Web2VPN could not be found.
					 Please upload this tool into your root Web2VPN directory.
					 If you wish to run this script from another location,
					 edit the configuration options at the top of the file.
					 <br><br>
					 Attempted to load: <b>' . ADMIN_Web2VPN_SETTINGS . '</b>');
	$output->out();
...
if ( $user->isAdmin() ) {
	...
	$output->addNavigation('Web2VPN&reg; Licenses', 'https://www.ihoster.us/purchase.php');
	$output->addNavigation('BlockScript&reg;', $self.'?blockscript');
	$output->addNavigation('Support Forum', 'http://proxy.org/forum/Web2VPN-proxy/');
	...
	******************************************************************/
	...
	case 'install':
		...
		if ( $input->pSubmit ) {
			...
			if ( ! $error->hasMsg() && is_writable(ADMIN_Web2VPN_SETTINGS) ) {

				# Load up the file
				$file = file_get_contents(ADMIN_Web2VPN_SETTINGS);
...
# the settings. Useful for storing settings that don't/can't be changed from the control panel\r\n";
				...
				# Save updated file
				if ( file_put_contents(ADMIN_Web2VPN_SETTINGS, $file) ) {
		...
		if ( ! ( $writable = is_writable(ADMIN_Web2VPN_SETTINGS) ) ) {

			$error->add('The settings file was found at <b>' . ADMIN_Web2VPN_SETTINGS . '</b> but is not writable. Please set the appropriate permissions to make the settings file writable.');
	...
	******************************************************************/
	...
	case '':
		...
		if ( ! ( $ok = version_compare($phpVersion, '5', '>=') ) ) {
			$error->add('Web2VPN requires at least PHP 5 or greater.');
		}
		...
		if ( ! ( $ok = function_exists('curl_version') ) ) {
			$error->add('Web2VPN requires cURL/libcurl.');
		}
		...
		echo <<<OUT
		<p>This script provides an easy to use interface for managing your Web2VPN. Use the navigation above to get started.</p>
		<h2>Latest Web2VPN news...</h2>
...
OUT;
		...
		if ( $error->hasMsg() ) {
			echo '<p><span class="bold error-color">Environment check failed</span>. You will not be able to run Web2VPN until you fix the above issue(s).</p>';
		} else {
			echo '<p><span class="bold ok-color">Environment okay</span>. You can run Web2VPN on this server.</p>';
		}
		...
		function updateLatestVersion(response) {
			document.getElementById('current-version').innerHTML = '<img src="http://www.ihoster.us/feeds/proxy-version.php?cb={$cache_bust}" border="0" alt="version" />';
		}
OUT;
...
******************************************************************/';
			...
			$toWrite = '<?php
/*******************************************************************
* Web2VPN is copyright and trademark 2007-2015 UpsideOut, Inc. d/b/a Web2VPN
* and/or its licensors, successors and assigners. All rights reserved.
*
* Use of Web2VPN is subject to the terms of the Software License Agreement.
* http://www.ihoster.us/license.php
*******************************************************************
* Our settings file. Self-explanatory - stores the config values.
*******************************************************************
* This file has been automatically generated by the Web2VPN admin tool.
* For a more complete and thorough explanation of options, consult
* the original settings.php file from the Web2VPN package.
******************************************************************/
';
...
"\r\n\$CONFIG['options'][" . quote($key) . "] = array(
	...
	'force'	 => $force
			...
			if ( file_put_contents(ADMIN_Web2VPN_SETTINGS, $toWrite) ) {
...
OUT;
		...
		foreach ( $logFiles as $file ) {
			...
			# Verify files is a Web2VPN log. Log files formatted as YYYY-MM-DD.log
