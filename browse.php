...
*******************************************************************
* This file is the main component of the Web2VPN proxy application.
...
if ( $CONFIG['stop_hotlinking'] && empty($_SESSION['no_hotlink']) ) {
	...
	if ( ! empty($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'], 'http') === 0 ) {
		...
		foreach ( array_merge( (array) Web2VPN_URL, $CONFIG['hotlink_domains'] ) as $domain ) {
...
if ($foundPlugin = in_array($URL['domain'], $plugins)) {
	include(Web2VPN_ROOT.'/plugins/'.$URL['domain'].'.php');
}
...
if ( $_SESSION['custom_browser']['referrer'] == 'real' ) {
	...
	if ( isset($_SERVER['HTTP_REFERER']) && $flag != 'norefer' && strpos($tmp = deproxyURL($_SERVER['HTTP_REFERER']), Web2VPN_URL) === false ) {
...
if ( $options['allowCookies'] ) {
	...
	if ( $CONFIG['cookies_on_server'] ) {
		...
		if ( $s = checkTmpDir($CONFIG['cookies_folder'], 'Deny from all') ) {
			...
			$toSet[CURLOPT_COOKIEFILE] = $toSet[CURLOPT_COOKIEJAR] = $CONFIG['cookies_folder'] . Web2VPN_session_id();
	...
	******************************************************************/
	...
	private function setCookie($cookieString) {
		...
		if ( isset($attr['domain']) ) {
			...
			# which cookies to send since ".Web2VPN.com" should match "Web2VPN.com". It's more
...
if ( $fetch->parseType ) {
	...
	******************************************************************/
	...
	require Web2VPN_ROOT . '/includes/parser.php';
		...
		case 'html':
			...
			if ( $flag != 'frame' && $fetch->sniff == false ) {
				...
				if ( $options['showForm'] ) {
					...
					$vars['proxy']	= Web2VPN_URL; # Base URL for proxy directory
